...
#' @return Measure
`_approximate_divergence_get_inner_measure` <- function(
  privacy_measure
) {
...
`_extrinsic_measure_descriptor` <- function(
  measure
) {
  # No type arguments to standardize.
  log_ <- new_constructor_log("_extrinsic_measure_descriptor", "measures", new_hashtab(
    list("measure"),
    list(measure)
  ))
  ...
  # Call wrapper function.
  output <- .Call(
    "measures___extrinsic_measure_descriptor",
    measure,
    log_, PACKAGE = "opendp")
  output
}
...
#'
#' [approximate in Rust documentation.](https://docs.rs/opendp/0.15.0-nightly.20260530.1/opendp/measures/struct.Approximate.html)
#'
#' **Proof Definition:**
#'
...
#'
#' The privacy profile should implement a well-defined \eqn{\delta(\epsilon)} curve:
#'
#' * monotonically decreasing
#' * rejects epsilon values that are less than zero or nan
#' * returns delta values only within \eqn{[0, 1]}
#'
#' @concept measures
#' @param curve A privacy curve mapping epsilon to delta
#' @export
new_privacy_profile <- function(
  curve
) {
  assert_features("contrib", "honest-but-curious")

  # Standardize type arguments.
  .T.curve <- rt_canon("f64")

  log_ <- new_constructor_log("new_privacy_profile", "measures", new_hashtab(
    list("curve"),
    list(curve)
  ))
